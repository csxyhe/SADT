...
def main():
    ...
    parser.add_argument('-opt', type=str, help='Path to option YMAL file.', default='./options/train/train_SADT.yml')
    ...
    lpips_metric_alex = lpips.LPIPS(net='alex')
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if (current_step % opt['train']['val_freq'] == 0) and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    cropped_gt_img = gt_img[crop_size:-crop_size, crop_size:-crop_size, :] # 裁剪掉效果不佳的边框
                    avg_lpips += lpips_metric_alex.forward(torch.clamp(visuals['Denoised'], 0, 1), torch.clamp(visuals['GT'], 0, 1), normalize=True).item()
                ...
                # log
                logger.info('# Validation # PSNR: {:.3f}, LPIPS: {:.5f}.'.format(avg_psnr, avg_lpips))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.3f}, lpips: {:.5f}.'.format(
