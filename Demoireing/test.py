...
def load_checkpoint(model):
    ...
    # remove unnecessary 'module.' for parallel pre-trained model (if neccessary)
    model_state_dict_clean = OrderedDict()  
    for k, v in model_state_dict.items():
        if k.startswith('module.'):
            model_state_dict_clean[k[7:]] = v
        else:
            model_state_dict_clean[k] = v    
