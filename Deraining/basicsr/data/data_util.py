...
#         return sorted(list(scandir(folder)))

#     with ThreadPoolExecutor() as executor:
#         input_paths, gt_paths = executor.map(scan_folder, [input_folder, gt_folder])
...
#         paths.append({f'{input_key}_path': osp.join(input_folder, input_path),
#                       f'{gt_key}_path': osp.join(gt_folder, gt_path)})
#     return paths

def paired_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders using generators for memory efficiency.

    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder].
        keys (list[str]): A list of keys identifying folders. The order should
            be consistent with folders, e.g., ['lq', 'gt'].
        filename_tmpl (str): Template for each filename.

    Yields:
        dict: A dictionary containing the paths of input and GT images.
    """
    assert len(folders) == 2, (
        'The len of folders should be 2 with [input_folder, gt_folder]. '
        f'But got {len(folders)}')
    ...
    def scan_folder(folder):
        # Instead of returning a list, yield each file path one by one
        for path in sorted(scandir(folder)):
            yield path

    # Using generators for both input and gt folders
    input_paths = scan_folder(input_folder)
    gt_paths = scan_folder(gt_folder)

    for input_path, gt_path in zip(input_paths, gt_paths):
        yield {
            f'{input_key}_path': osp.join(input_folder, input_path),
            f'{gt_key}_path': osp.join(gt_folder, gt_path)
        }
